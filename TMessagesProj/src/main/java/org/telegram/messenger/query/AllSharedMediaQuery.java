    ...
    private static void getMediaCountDatabase(final long uid, final int type, final int classGuid) {
        MessagesStorage.getInstance().getStorageQueue().postRunnable(new Runnable() {
            ...
            public void run() {
                try {
                    int count = -1;
                    SQLiteCursor cursor = MessagesStorage.getInstance().getDatabase().queryFinalized(String.format(Locale.US, "SELECT count FROM media_counts_v2 type = %d LIMIT 1", type));
                    if (cursor.next()) {
                    ...
                    if (count == -1 && lower_part == 0) {
                        cursor = MessagesStorage.getInstance().getDatabase().queryFinalized(String.format(Locale.US, "SELECT COUNT(mid) FROM media_v2 type = %d LIMIT 1", type));
                        if (cursor.next()) {
                            count = cursor.intValue(0);
                        }
                        cursor.dispose();
            ...
            public void run() {
                ...
                try {
                    ...
                    if ((int) uid != 0) {
                        ...
                        cursor = database.queryFinalized(String.format(Locale.US, "SELECT start FROM media_holes_v2 type = %d AND start IN (0, 1)",  type));
                        if (cursor.next()) {
                            isEnd = cursor.intValue(0) == 1;
                            cursor.dispose();
                        } else {
                            cursor.dispose();
                            cursor = database.queryFinalized(String.format(Locale.US, "SELECT min(mid) FROM media_v2 type = %d ", type));
                            if (cursor.next()) {
                        ...
                        if (messageMaxId != 0) {
                            ...
                            cursor = database.queryFinalized(String.format(Locale.US, "SELECT end FROM media_holes_v2  type = %d AND end <= %d ORDER BY end DESC LIMIT 1",  type, max_id));
                            if (cursor.next()) {
                            ...
                            cursor.dispose();
                            if (holeMessageId > 1) {
                                cursor = database.queryFinalized(String.format(Locale.US, "SELECT data, mid FROM media_v2 mid > 0 AND mid < %d AND mid >= %d AND type = %d ORDER BY date DESC, mid DESC LIMIT %d",  messageMaxId, holeMessageId, type, countToLoad));
                            } else {
                                cursor = database.queryFinalized(String.format(Locale.US, "SELECT data, mid FROM media_v2  mid > 0 AND mid < %d AND type = %d ORDER BY date DESC, mid DESC LIMIT %d", messageMaxId, type, countToLoad));
                            }
                        } else {
                            long holeMessageId = 0;
                            cursor = database.queryFinalized(String.format(Locale.US, "SELECT max(end) FROM media_holes_v2  type = %d",  type));
                            if (cursor.next()) {
                                holeMessageId = cursor.intValue(0);
                                if (channelId != 0) {
                                    holeMessageId |= ((long) channelId) << 32;
                            ...
                            cursor.dispose();
                            if (holeMessageId > 1) {
                                cursor = database.queryFinalized(String.format(Locale.US, "SELECT data, mid FROM media_v2 mid >= %d AND type = %d ORDER BY date DESC, mid DESC LIMIT %d,%d",  holeMessageId, type, offset, countToLoad));
                            } else {
                                cursor = database.queryFinalized(String.format(Locale.US, "SELECT data, mid FROM media_v2  mid > 0 AND type = %d ORDER BY date DESC, mid DESC LIMIT %d,%d",  type, offset, countToLoad));
                            }
                        }
                    } else {
                        isEnd = true;
                    ...
                    while (cursor.next()) {
                        NativeByteBuffer data = new NativeByteBuffer(0);
                        if (data != null) {
    ...
    public static void loadMusic(final long uid, final int max_id) {
        MessagesStorage.getInstance().getStorageQueue().postRunnable(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    SQLiteCursor cursor = MessagesStorage.getInstance().getDatabase().queryFinalized(String.format(Locale.US, "SELECT data, mid FROM media_v2 mid < %d AND type = %d ORDER BY date DESC, mid DESC LIMIT 1000", max_id, MEDIA_MUSIC));
